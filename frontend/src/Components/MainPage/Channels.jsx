...
const NotRemovableChannel = ({
  ...
  return (
    <Button onClick={() => handleSelect(Number(id))} variant={isCurrentChannel(id) ? 'primary' : 'ligth'} className="w-100 rounded-0 text-start">
...
const RemovableChannel = ({
  name, id, isCurrentChannel, handleSelect, handleRemove,
  handleRename,
}) => {
  const { t } = useTranslation();
  ...
  return (
    <Dropdown as={ButtonGroup} className="d-flex">
      ...
      />
      <Dropdown.Toggle split variant={isCurrentChannel(id) ? 'primary' : 'ligth'} id="dropdown-split-basic">
  ...
  return (
    <Col xs={4} md={2} className="border-end px-0 bg-light flex-column h-100 d-flex">
      <div className="d-flex mt-1 justify-content-between mb-2 ps-4 pe-2 p-4">
        ...
        <Button onClick={handleAdd} variant="outline-primary" className="p-0 d-flex btn-group-vertical">
          <svg
            version="1.1"
